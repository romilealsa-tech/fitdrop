        ...
        {loading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[...Array(6)].map((_, i) => (
              <div key={i} className="bg-[#1C1C1E] border border-[#2B2B2E] rounded-2xl p-6 animate-pulse">
                <div className="w-full h-32 bg-[#2B2B2E] rounded-xl mb-4" />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {products.map((product) => (
              <div
                key={product._id}
                className="bg-[#1C1C1E] border border-[#2B2B2E] rounded-2xl p-6 hover:border-[#2DD4BF] transition"
              >
                ...
                <div className="flex justify-between items-start mb-2">
                  <h4 className="font-semibold text-[#E8E8EA]">{product.name}</h4>
                  {product.tag && (
                    <span className="text-xs bg-[#2DD4BF] text-[#0D0D0F] px-2 py-1 rounded-full font-bold">
                      {product.tag}
                    </span>
                  )}
                </div>
                <p className="text-[#2DD4BF] text-sm mb-4 font-medium">{product.price}</p>
                <button
                  onClick={() => addToCart({ ...product, id: product._id, store: store.name })}
                  className="w-full bg-[#2DD4BF] text-[#0D0D0F] py-2 rounded-full text-sm font-bold hover:bg-[#22b8a4] transition"
                >
                  Add to Cart
                </button>
              </div>
