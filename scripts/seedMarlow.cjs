// Plain CommonJS version (no build step needed) — seeds the "Marlow" demo
// store only. Safe to re-run: only ever touches slug === "marlow".
...
async function seed() {
  if (!MONGODB_URI) {
    console.log("ERROR: MONGODB_URI not set. Run with: node --env-file=.env.local scripts/seedMarlow.cjs")
    process.exit(1)
  }
